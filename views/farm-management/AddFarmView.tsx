...
import { addFarm, addfarm } from "@/requests/farm";
...
export default function AddFarmView() {
  const { t } = useLanguage()
    ...
    const {
      ...
      const onSubmit = async (data: FormData) => {
        try {
            await addFarm(data as Farm)
            toast.success(t('farm.added'))
  ...
  return (
    <>
      <Row>
        <Col span={12}>
          <h2 className="text-2xl font-semibold">{t('farm.addFarm')}</h2>
        </Col>
      </Row>
          <form onSubmit={handleSubmit(onSubmit)}>
            ...
            <Col span={6} sm={6}>
                ...
                <div className="flex justify-between items-center mb-6">
                    <h3 className="text-primary text-3xl mt-8 mb-3">{ t('progressInfos') }</h3>
                    <Button
                            // variant="ghost"
                            type="button"
                            size="icon"
